...
const Upload = () => {
  ...
  // 🔥 New: Send image to Flask backend
  const handleViewResults = async () => {
    ...
    try {
      setLoading(true);
      toast.message("Processing image...", { description: "Generating predictions..." });

      const formData = new FormData();
      formData.append("file", file);

      // Optional product metadata (customize as needed)
      const metadata = {
      ...
      // Save results locally (for /results page)
      localStorage.setItem("uploadedImage", selectedImage!);
      localStorage.setItem("predictedPrice", data.predicted_price ?? "N/A");
      localStorage.setItem("cnnOutput", JSON.stringify(data.cnn_output));
